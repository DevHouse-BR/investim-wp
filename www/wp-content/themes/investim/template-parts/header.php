...
if( is_singular() ) {
	...
	$display_search = get_post_meta( get_the_id(), '_search_display', true );

	$display_search_on_header = get_post_meta( get_the_id(), '_search_display_on_header', false );

	$video_bg = get_post_meta( get_the_id(), '_tagline_video_bg', false );	
}
...
<header class="site-header<?php if( $header_display ) echo ' site-header-' . esc_attr( $header_display ); if( $header_filter ) echo ' site-header-no-filter'; ?>" role="banner" itemscope itemtype="http://schema.org/WPHeader"<?php echo $background_image; ?>>
	...
	<?php if( $display_search ) : 
		if ($display_search_on_header):	
			if ($video_bg) :?>
		<div class="cabec-video-container">
			<video id="cabec-video" preload="auto" autoplay loop>
				<source src="<?php echo $video_bg[0] ?>" type="video/mp4">
			</video>
			<div id="cabec-video-overlay"></div>
		</div>
			<?php endif; ?>	
		<?php endif; ?>		
	<?php endif; ?>
