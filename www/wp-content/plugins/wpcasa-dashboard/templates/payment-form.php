...
<?php if( empty( $payment_type ) ) : ?>
	...
	<div class="wpsight-alert alert payment-info">
...
<?php else: ?>
	...
	<form class="wpsight-dashboard-form payment-form" method="post" action="?">
		...
		<!-- Billing details -->
		<h2><?php _e( 'Billing details', 'wpcasa-dashboard' ) ?></h2>
		...
		<?php if( ! empty( $price ) && $price != 0 ) : ?>
			...
			<h2><?php _e( 'Payment Gateway', 'wpcasa-dashboard' ) ?></h2>
			...
			<?php if( is_array( $payment_gateways ) && count( $payment_gateways ) > 0 ) : ?>
				...
				<div class="payment-form-bottom">
					...
					<input type="submit" name="process-payment" class="payment-process" value="<?php esc_attr_e( 'Process Payment', 'wpcasa-dashboard' ); ?>" />
